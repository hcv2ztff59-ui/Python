...
def remove_mention(task_id: int, id_user_mentioned: int, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not mention:
        print(f"❌ ERRORE: Menzione specifica non trovata (task_id: {task_id}, user_id cercato: {id_user_mentioned if is_creator else current_user['id_utente']}).")
        raise HTTPException(status_code=404, detail="Menzione specifica non trovata")
